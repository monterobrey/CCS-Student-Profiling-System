          ...
          <table className="data-table">
            ...
            <tbody>
                  ...
                  <tr key={f.id} onClick={() => viewDetails(f)} className="clickable-row">
                    <td>
                      <div className="student-cell">
                        <div className="s-avatar" style={{ background: color }}>{f.first_name.charAt(0)}</div>
                        <div>
                          <p className="s-name">
                    ...
                    <td>
                      <div className="units-breakdown">
                        ...
                        <div className="wl-bar-sm"><div className="wl-fill-sm" style={{ width: Math.min(load / 50 * 100, 100) + '%' }}></div></div>
                      </div>
                    </td>
                    <td>
                    ...
                    {(isSecretary || isDeanOrChair) && (
                      <td onClick={e => e.stopPropagation()}>
                        <div className="action-btns">
                          ...
                          <button className="action-btn delete" onClick={() => confirmArchive(f)} title="Archive">
              ...
              {paginatedFaculty.length === 0 && !isLoading && (
                <tr><td colSpan={isSecretary || isDeanOrChair ? 6 : 5} className="empty-row">No faculty members found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
            ...
            <div className="modal modal-lg" onClick={e => e.stopPropagation()}>
              <div className="modal-header">
                <div className="modal-student-meta">
                  <div className="s-avatar lg" style={{ background: color }}>{viewingFaculty.first_name.charAt(0)}</div>
                  <div>
                    <h3>
                    ...
                    <p className="modal-sub">{viewingFaculty.position} · {deptName}</p>
                  </div>
                </div>
              ...
              <div className="modal-body profile-body">
                <div className="profile-section">
                  ...
                  <div className="detail-rows">
                    {viewingFaculty.title && <div className="detail-row"><span className="detail-key">Title</span><span className="detail-val">{viewingFaculty.title}</span></div>}
                    <div className="detail-row"><span className="detail-key">Full Name</span><span className="detail-val">{viewingFaculty.last_name}, {viewingFaculty.first_name} {viewingFaculty.middle_name || ''}</span></div>
                    <div className="detail-row"><span className="detail-key">Email Address</span><span className="detail-val">{email}</span></div>
                    <div className="detail-row"><span className="detail-key">Position</span><span className="detail-val"><span className="code-badge">{viewingFaculty.position}</span></span></div>
                  </div>
                </div>
                ...
                <div className="profile-section">
                  ...
                  <div className="detail-rows">
                    ...
                    <div className="detail-row">
                      ...
                      <span className="detail-val">
                        <div className="units-breakdown" style={{ justifyContent: 'flex-end' }}>
                          <span className="total-units-badge">{load} hrs</span>
                          <div className="wl-bar-sm" style={{ width: '60px' }}><div className="wl-fill-sm" style={{ width: Math.min(load / 50 * 100, 100) + '%' }}></div></div>
                        </div>
                      </span>
                    </div>
                    ...
                    <div className="detail-row"><span className="detail-key">Date Joined</span><span className="detail-val">{new Date(viewingFaculty.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span></div>
                  </div>
                </div>
                ...
                <div className="profile-section">
                  ...
                  <div className="detail-rows">
                    {viewingFaculty.expertise?.length > 0
                      ? viewingFaculty.expertise.map(e => <div key={e.id} className="detail-row"><span className="detail-val">{e.expertise_name}</span></div>)
                      : <div className="detail-row"><span className="detail-val" style={{ color: '#b89f90', fontStyle: 'italic' }}>No expertise recorded.</span></div>
                    }
                  </div>
                </div>
                ...
                {status === 'pending' && isSecretary && (
                  <div className="resend-row">
                    ...
                    <button className="resend-btn" onClick={() => resendSetup(viewingFaculty)} disabled={getResendCount(viewingFaculty.id) >= 3}>
            ...
            <div className="modal-body">
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <input value={form.first_name} onChange={e => setForm(p => ({...p, first_name: e.target.value}))} placeholder="First name" disabled={!!editingFaculty || saving} className={formErrors.first_name ? 'error-input' : ''} />
                  {formErrors.first_name && <span className="field-error">{formErrors.first_name}</span>}
                </div>
                <div className="form-group">
                  <label>Last Name <span className="req">*</span></label>
                  <input value={form.last_name} onChange={e => setForm(p => ({...p, last_name: e.target.value}))} placeholder="Last name" disabled={!!editingFaculty || saving} className={formErrors.last_name ? 'error-input' : ''} />
                ...
                <div className="form-group full-span">
                  ...
                  <input value={form.email} onChange={e => setForm(p => ({...p, email: e.target.value}))} type="email" placeholder="faculty@school.edu.ph" disabled={!!editingFaculty || saving} className={formErrors.email ? 'error-input' : ''} />
                  {formErrors.email && <span className="field-error">{formErrors.email}</span>}
                </div>
                <div className="form-group">
                  <label>Department <span className="req">*</span></label>
                  <select value={form.department_id} onChange={e => setForm(p => ({...p, department_id: e.target.value}))} disabled={saving} className={formErrors.department_id ? 'error-input' : ''}>
                    <option value="">Select Department</option>
                    {departments.map(d => <option key={d.id} value={d.id}>{d.department_name}</option>)}
                  </select>
                  {formErrors.department_id && <span className="field-error">{formErrors.department_id}</span>}
                </div>
                <div className="form-group">
